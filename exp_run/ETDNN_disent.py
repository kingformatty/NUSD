...
class ECAPA_TDNN_small(nn.Module):
    def __init__(self, in_channels=80, channels=128, att_dim=64, embd_dim=128, class_num=1, num_spk=107):
        super().__init__()
        self.layer1 = Conv1dReluBn(in_channels, channels, kernel_size=5, padding=2)
        self.layer2 = SE_Res2Block(channels, kernel_size=3, stride=1, padding=2, dilation=2, scale=8)
        self.layer3 = SE_Res2Block(channels, kernel_size=3, stride=1, padding=3, dilation=3, scale=8)
        ...
        self.linear3 = nn.Linear(embd_dim, class_num)
        self.bn3 = nn.BatchNorm1d(class_num)
        self.linear4 = nn.Linear(embd_dim,num_spk)
        self.bn4 = nn.BatchNorm1d(num_spk)
    ...
    def forward(self, x):
        ...
        out_pred = self.sigmoid1(out_pred)

        spk_pred = self.bn4(self.linear4(out_embd))

        return out_pred,spk_pred,out_embd


class ECAPA_TDNN_small_raw(nn.Module):
    def __init__(self, in_channels=1, channels=128, att_dim=64, embd_dim=128, class_num=1, num_spk=107):
        super().__init__()
        self.layer1 = Conv1dReluBn(in_channels, channels, kernel_size=1024, stride=512)
        self.layer2 = SE_Res2Block(channels, kernel_size=3, stride=1, padding=2, dilation=2, scale=8)
        self.layer3 = SE_Res2Block(channels, kernel_size=3, stride=1, padding=3, dilation=3, scale=8)
        self.layer4 = SE_Res2Block(channels, kernel_size=3, stride=1, padding=4, dilation=4, scale=8)
...
if __name__ == '__main__':
    ...
    ecapa_small_80mel = ECAPA_TDNN_small(in_channels=80)
