...
class CustomRaw3(nn.Module):
    ...
    def forward(self, net_input):
        ...
        x = torch.transpose(x, 1, 2)
        x, hidden_state = self.lstm(x)
        mdd = self.fc(x[:, -1, :].reshape(batch, -1))
        spk = self.fc2(x[:, -1, :].reshape(batch, -1))
        return mdd,spk,hidden_state[0][-1]
...
class CustomWav2vec2_1(nn.Module):
    def __init__(self):
        super(CustomWav2vec2_1, self).__init__()

        self.input_layer = nn.LSTM(input_size=768,
                            hidden_size=256,
                            num_layers=1,
                            batch_first=True,
                            bidirectional=False)
...
# class CustomWav2vec2_2(nn.Module):
#     def __init__(self,loss_alpha):
#         super(CustomWav2vec2_1, self).__init__()
...
#                                  activation='sigmoid',
#                                  normalisation=None)
#         self.spk_layer = nn.Sequential(
#                             FullyConnected(in_channels=256,
#                                         out_channels=107,
#                                         activation='global',
#                                         normalisation=None),
#                             GradientReversal(alpha=loss_alpha)
#                             )
...
#         x, hidden_state = self.lstm(x)
#         mdd = self.fc(x[:, -1, :].reshape(batch, -1))
#         spk = self.spk_layer(x[:, -1, :].reshape(batch, -1))
...
if __name__ == '__main__':
    ...
    depaudionet_model = CustomComparE16()
    pytorch_total_params_depaudionet= sum(p.numel() for p in depaudionet_model.parameters() if p.requires_grad)
